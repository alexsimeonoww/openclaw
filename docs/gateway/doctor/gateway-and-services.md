...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    Plain Doctor inspection checks the installed supervisor config (launchd/systemd/schtasks) for missing or outdated defaults (for example systemd network-online dependencies and restart delay) and can offer an interactive repair. Explicit repair maintenance preserves the installed launcher and environment. Before a Linux maintenance stop, it backs up and refreshes outdated OpenClaw unit policy, confirms `daemon-reload`, and verifies that the effective `TimeoutStopSec` covers drain and cleanup (currently 330 seconds). Operator drop-ins remain unchanged. A short or unknown resident shutdown budget uses bounded lifecycle drain before stopping. At the update deadline, outstanding write custody refuses the stop with its owner phase; remaining admitted turns produce a warning and the stop proceeds. Run `openclaw gateway install --force` from the intended installation to replace the launcher and managed environment.
    ...
    - `openclaw doctor --yes` accepts default non-service repair prompts and enters maintenance with the Linux policy refresh described above.
    - `openclaw doctor --fix` applies recommended repairs without prompts (`--repair` is an alias; `--yes` also enters repair maintenance). It stops the matching managed Gateway before plugin or mutable-state inspection, verifies repairs, and restarts the same service once, even when no changes are needed. It preserves the installed launcher and environment, leaves services confirmed offline before maintenance offline, and refuses to stop an ancestor Gateway. Plain inspection does not enter maintenance, and custom state directories do not adopt native services.
    ...
    - `openclaw doctor --fix --force` uses the same policy refresh and stop-timeout verification. Use `openclaw gateway install --force` to replace the launcher and environment; operator-owned systemd drop-ins remain unchanged.
