...
For the sampled 123-envelope inventory, 320 is the smallest whole-second budget that lets the recalibrated fallback fit every current matrix cap. At 319 seconds it needs 43 extension rows; at 320 it needs 42. Blacksmith, hybrid and GitHub compact plans retain 76, 72 and 80 rows respectively, including two dist descriptors outside the Node matrix. Final PR Node counts are therefore 116, 112 and 120. The 50-row extension, 80-row compact and final 120-row PR caps remain unchanged. Canonical pushes do not append changed-extension envelopes and retain their 64-row final cap. The conservative registration ceiling is unchanged because no row cap increases.

Median rates are estimates, not elapsed-time guarantees. Replaying the selected fallback layout against the largest matching observed child spans gives a 520.688-second combined envelope sum; the slowest individual envelope is 508.783 seconds. Whole-config observations can have different file inventories across source revisions. These are forecasts, not measured combined-job walls. The original job in run 35490342736 bundled 17 envelopes into 2,015.674 seconds of child spans and 2,049 seconds of wall time despite a 240-second prediction. Native PR CI must verify the interim improvement toward the ten-minute PR objective. Exact config/include-set observations and successful PR-run ingestion in the timing refit remain follow-up work; narrow PR samples must not prune unrelated observations merely because they were not selected.
