...
and verifies readiness. It preserves the installed launcher and environment and does
...
but preserves the installed launcher and environment. The Linux policy refresh
described above also applies. Force does not bypass service
...
| `--repair` / `--fix`            | Apply recommended repairs while coordinating maintenance with the matching managed Gateway (`--fix` is an alias). Refresh outdated Linux unit policy before stopping; preserve the installed launcher and environment. |
| `--force`                       | Allow aggressive repair choices. Alone, remains guided; with `--fix`, `--repair`, or `--yes`, preserves the installed launcher and environment.                                                                        |
