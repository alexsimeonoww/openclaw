...
- `doctor --fix --non-interactive` preserves the installed Gateway launcher and environment, including during update repair. Before stopping a Linux service, it backs up and refreshes outdated OpenClaw unit policy, confirms `daemon-reload`, and verifies the effective shutdown timeout. Operator drop-ins remain unchanged. Short or unknown resident shutdown budgets use bounded lifecycle drain; outstanding write custody refuses the stop, while interrupted turns produce a warning. Run `openclaw gateway install` for a missing service, or `openclaw gateway install --force` from the intended installation to replace its launcher and managed environment.
